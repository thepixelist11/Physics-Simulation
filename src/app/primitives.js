...
class Point {
  ...
  #_initialRadius
  #_initialColor
  constructor(position, mass, radius = 5, color = Color.BLACK) {
  ...
  set x(x) {
    if(typeof x === 'number'){
      this.#_position = new Vector2(x, this.#_position.x)
    } else {
  ...
  set y(y) {
    if(typeof y === 'number'){
      this.#_position = new Vector2(this.#_position.y, y)
    } else {
  ...
  set lastPosition(newLastPosition) {
    if(newLastPosition instanceof Vector2) {
  ...
  draw(ctx) {
    drawPoint(ctx, this.position, this.radius, this.color)
  }
